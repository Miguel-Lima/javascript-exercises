const states = [
  ...
  const citiesMap = cities.map ((city) => {
      const regionsFind = regions.find ((regiao) => regiao.short === city.region);
      const statesFind = states.find ((estado) => estado.short === city.state);
      return {
          estado: statesFind.short,
          região: regionsFind.name,
          cidade: city.name
      }
  })
